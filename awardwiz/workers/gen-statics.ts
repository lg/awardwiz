...
const fr24Airports = await fetch("https://www.flightradar24.com/airports/list").then(async (airportsRes) => airportsRes.json()) as FR24Airports
...
const airports = Object.values(fr24Airports).map((fr24Airport): Airport | undefined => {
  ...
  } satisfies Airport : undefined
}).filter((airport): airport is Airport => !!airport).sort((a, b) => a.popularity > b.popularity ? -1 : 1)

writeFileSync("awardwiz/airports.json", JSON.stringify(airports, undefined, 2))
