...
export const arkalisInterceptor = (arkalis: ArkalisCore) => {
  ...
  void arkalis.client.Fetch.enable({ handleAuthRequests: !!onAuthReq }).then(() => {
    if (onAuthReq)
      arkalis.client.Fetch.on("authRequired", (authReq) => onAuthReq(arkalis.client, authReq))

    try {
      return arkalis.client.Fetch.on("requestPaused", async (requestPausedEvent) => {
      ...
      console.log("werjhwejrhwej")
      throw new Error("werewrew")
      //console.log(error)
    }
  })
