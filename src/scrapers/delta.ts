...
export const runScraper: AwardWizScraper = async (sc, query) => {
  ...
  sc.log("selected date, submitting search")
  ...
  const result = await Promise.race([
    ...
    sc.page.getByRole("button", { name: "Continue" }).waitFor().then(() => "ok").catch((e: Error) => e.message),
  ...
  void sc.page.getByRole("button", { name: "Continue" }).click()  // async
